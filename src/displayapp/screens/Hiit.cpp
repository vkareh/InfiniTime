...
Hiit::Hiit(Controllers::MotorController& motorController,
...
uint8_t Hiit::GetRemainingSeconds() const {
  TickType_t elapsed = xTaskGetTickCount() - phaseStartTime;
  bool isActivePhase = (currentState == State::Active ||
                        (currentState == State::Paused && stateBeforePause == State::Active));
